...
const STATUS_CONFIG = {
  PENDING: { label: 'Chờ thanh toán', icon: Clock, color: 'bg-amber-50 text-amber-600 border-amber-100', dot: 'bg-amber-400' },
  PAID: { label: 'Đã thanh toán', icon: CheckCircle, color: 'bg-green-50 text-green-600 border-green-100', dot: 'bg-green-400' },
  FAILED: { label: 'Hết hạn', icon: XCircle, color: 'bg-red-50 text-red-500 border-red-100', dot: 'bg-red-400' },
};
...
export const Transactions = () => {
  ...
  const fetchBids = async () => {
    ...
    try {
      ...
      toast.error('Không tải được danh sách trả giá');
    } finally {
  ...
  const handlePay = async (transactionId) => {
    if (!window.confirm('Xác nhận thanh toán? Quá trình này sẽ trừ tiền từ số dư trong ví của bạn.')) return;
    setPayingId(transactionId);
    try {
      const result = await payTransaction(transactionId);
      toast.success(`Thanh toán thành công! Số dư mới: ${Number(result.data?.currentBalance).toLocaleString()} đ`);
      fetchTransactions();
    } catch (err) {
    ...
    if (bids.length === 0) {
      return (
        <div className="bg-white rounded-[32px] border border-dashed border-gray-200 py-20 text-center">
          ...
          <p className="font-black text-gray-400 uppercase tracking-widest text-[10px]">Chưa tham gia phiên đấu giá nào</p>
          ...
          return (
            <div key={item.id} className="bg-white rounded-[24px] border border-gray-100 p-6 shadow-sm hover:shadow-xl hover:shadow-primary-500/5 transition-all group relative overflow-hidden">
              ...
              <div className={`absolute top-4 right-4 px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest
                ...
                {isEventActive ? 'Trực tiếp' : (item.event?.status === 'PENDING' ? 'Sắp mở' : 'Đã kết thúc')}
              </div>
              ...
              <div className="bg-gray-50 rounded-xl p-4 mb-5 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-gray-500 font-bold uppercase">Giá hiện tại</span>
                  <span className="font-black text-gray-900">{Number(item.current_price).toLocaleString()} đ</span>
                </div>
                <div className="flex justify-between items-center border-t border-gray-200/60 pt-3">
                  <span className="text-[10px] text-primary-600 font-bold uppercase">Giá của tôi</span>
                  <div className="text-right">
                    <span className="font-black text-primary-600 block">{Number(myHighestBid).toLocaleString()} đ</span>
                    {isEventActive && (
                      <span className={`text-[9px] font-bold uppercase tracking-widest ${isHighestBidder ? 'text-green-500' : 'text-red-500'}`}>
                        {isHighestBidder ? 'Đang dẫn đầu!' : 'Khách khác trả cao hơn'}
                      </span>
                    )}
                  </div>
                </div>
              ...
              <Link
               ...
               >
                Vào Phòng Đấu Giá <ArrowRight className="w-4 h-4" />
              </Link>
            </div>
          );
        })}
      ...
      <>
        ...
        <div className="flex items-center gap-2 mb-6 flex-wrap">
          {['ALL', 'PENDING', 'PAID', 'FAILED'].map(s => (
            ...
            >
              {s === 'ALL' ? 'Tất Cả Biên Lai' : STATUS_CONFIG[s].label}
            </button>
          ))}
        </div>
        ...
        {filteredTxns.length === 0 ? (
          <div className="bg-white rounded-[32px] border border-dashed border-gray-200 py-20 text-center">
            ...
            <p className="font-black text-gray-400 uppercase tracking-widest text-[10px]">Không có sản phẩm nào trong mục này.</p>
          </div>
        ) : (
                      ...
                      <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div>
                          <p className="font-black text-gray-900 text-lg uppercase italic tracking-tight truncate">
                            {txn.item?.name || 'Sản phẩm ẩn'}
                          </p>
                          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">
                            Phiên: {txn.item?.event?.title || 'Đang cập nhật'}
                          </p>
                        ...
                        <div className="text-right flex-shrink-0">
                          <p className="text-2xl font-black text-gray-900">{Number(txn.amount).toLocaleString()} đ</p>
                          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Giá thắng cuộc</p>
                        </div>
                      </div>
    ...
    <div className="min-h-screen bg-gray-50/50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="mb-10 text-center md:text-left flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-4xl font-black text-gray-900 tracking-tight mb-2">Trang Cá Nhân</h1>
            <p className="text-gray-500 font-medium">Theo dõi các phiên đang tham gia và quản lý biên lai chiến thắng.</p>
          </div>
        </div>
        ...
        <div className="flex items-center border-b border-gray-200 mb-8 overflow-x-auto no-scrollbar">
          ...
          >
            <Activity className="w-5 h-5" /> Lịch Sử Tham Gia
          </button>
          <button
          ...
          >
            <Trophy className="w-5 h-5" /> Đã Trúng & Quản Lý Thanh Toán
          </button>
        </div>
